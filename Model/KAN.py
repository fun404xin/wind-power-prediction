...
class KANLinear(torch.nn.Module):
    def __init__(
        ...
        base_activation=torch.nn.ReLU,  # 基础激活函数，默认为 SiLU（Sigmoid Linear Unit）
        grid_eps=0.02,
    ...
    def regularization_loss(self, regularize_activation=1.0, regularize_entropy=1.0):
        l2_fake = self.spline_weight.pow(2).mean(-1)
        regularization_loss_activation = l2_fake.sum()
        p = l2_fake / regularization_loss_activation
